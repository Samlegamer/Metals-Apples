...
public class MetalApple {
    ...
    public static Item getItemByName(String name) {
        if(BuiltInRegistries.ITEM.get(Identifier.fromNamespaceAndPath(MODID, name)).isPresent())
        {
            return BuiltInRegistries.ITEM.get(Identifier.fromNamespaceAndPath(MODID, name)).get().value();
        }
        else
        {
            return Items.APPLE;
